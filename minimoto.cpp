...
void MiniMoto::I2CWriteBytes(byte addr, byte *buffer, byte len)
{
  // Send start condition
    ...
    if ((TWSR & 0xF8) != 0x08) {
        DEBUG_LOG("Failed to send START condition");
        
        DEBUG_LOG("TWSR error code: %x", TWSR & 0xF8);
        return;
    }

    // Load slave address
    TWDR = _addr;
    TWCR = CLEAR_TWINT | (1 << TWEN);  // Clear TWINT to transmit
    while (!(TWCR & (1 << TWINT)));  // Wait for address to transmit
    if ((TWSR & 0xF8) != 0x18) {
        DEBUG_LOG("Failed to send SLA+W (slave address write)");
        return;
    }
    ...
    while (!(TWCR & (1 << TWINT)));  // Wait for address to transmit
    if ((TWSR & 0xF8) != 0x28) {
        DEBUG_LOG("Failed to send data address");
        return;
    }
    ...
    if ((TWSR & 0xF8) == 0x30) { // Check for NACK
        DEBUG_LOG("NACK %d", i);
        return;
    }
}
